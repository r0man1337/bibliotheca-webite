  ...
  <section>
    <div>
      ...
      <div  v-if="!$fetchState.pending" class="flex flex-wrap">
        <div v-for="(adventurer, index) in adventurers" :key="index" class="w-80 ">
  ...
  setup(props, context) {
    ...
    const query = ref(gql`
      query walletQuery($offset: Int!) {
          wallets (orderBy: bagsHeld, orderDirection: desc, first: 100, skip: $offset, where: { bagsHeld_not:null }) {
            id
            address
            realmsHeld
            bagsHeld
